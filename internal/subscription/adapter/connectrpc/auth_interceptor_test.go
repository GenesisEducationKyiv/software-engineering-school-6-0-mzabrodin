...
func (s *AuthInterceptorSuite) TestEmptyAPIKeyPassesThrough() {
	...
	next := func(ctx context.Context, _ connect.AnyRequest) (connect.AnyResponse, error) {
		called = true
		return connect.NewResponse(&appv1.SubscribeResponse{}), nil
	}

	wrapped := NewAuthInterceptor("")(next)
	_, err := wrapped(s.T().Context(), connect.NewRequest(&appv1.SubscribeRequest{}))
...
func (s *AuthInterceptorSuite) TestUnprotectedProcedurePassesThrough() {
	...
	// connect.NewRequest carries an empty procedure, which is not in protectedProcedures,
	// so the interceptor must not require a key.
